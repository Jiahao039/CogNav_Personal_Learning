...
class MaskDINODecoder(nn.Module):
    ...
    def forward(self, x, mask_features, masks, targets=None, target_queries=None, target_vlp=None, task='seg', extra={}):
        ...
        refpoint_embed = refpoint_embed_undetach.detach()
        outputs_class, outputs_mask,decoder_output = self.forward_prediction_heads(tgt_undetach.transpose(0, 1), mask_features, do_seg)
        tgt = tgt_undetach.detach()
        if self.learn_tgt:
            tgt = self.query_feat.weight[None].repeat(bs, 1, 1)
        interm_outputs=dict()
        ...
        # direct prediction from the matching and denoising part in the begining
        if self.initial_pred:
            outputs_class, outputs_mask,decoder_output = self.forward_prediction_heads(tgt.transpose(0, 1), mask_features, self.training and do_seg)
        ...
        for i, output in enumerate(hs):
            outputs_class, outputs_mask,decoder_output = self.forward_prediction_heads(output.transpose(0, 1), mask_features, (self.training or (i == len(hs)-1)) and do_seg)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            decoder_outputs.append(decoder_output)
        # iteratively box prediction
    ...
    def forward_prediction_heads(self, output, mask_features, pred_mask=True):
        ...
        return outputs_class, outputs_mask,class_embed / (class_embed.norm(dim=-1, keepdim=True) + 1e-7)
