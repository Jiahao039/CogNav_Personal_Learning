...
class HabitatSimDepthSensor(DepthSensor, HabitatSimSensor):
    ...
    def __init__(self, config: DictConfig) -> None:
        ...
        # [补丁] 填充默认值
        if "normalize_depth" not in config:
            config.normalize_depth = True
...
class HabitatSim(habitat_sim.Simulator, Simulator):
    ...
    def __init__(self, config: DictConfig) -> None:
        ...
        for agent_config in self.habitat_config.agents.values():
            ...
            # [修复] 强制将配置转换为 OmegaConf 对象，使其支持属性访问 (如 .type, .height)
            # 这样既解决了 "dict object has no attribute type"，也解决了 Sensor 内部访问 .height 的问题
            for sensor_cfg in OmegaConf.to_container(agent_config.sim_sensors).values():
                if isinstance(sensor_cfg, dict):
                    sensor_cfg = OmegaConf.create(sensor_cfg)
